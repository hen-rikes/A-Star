...
void order_and_correct_survivor_list(const std::map<std::string, UInt>& survivors, std::vector<std::pair<std::string, UInt>>& survivors_ordered, UInt& eclipse_level_sum, UInt& mastery_level) {
    ...
    for (const auto& [name, e_level] : survivors) {
        ...
        survivors_ordered[survivor_order[index]-1] = std::make_pair(temp_name, e_level > 0 ? e_level - 1 : 0);
        index++;
    }

    eclipse_level_sum = 0;
    mastery_level = 8;
    for (const auto& [name, e_level] : survivors_ordered) {
    ...
    while (!WindowShouldClose()) {
        ...
        if (state == LOADING_STATE) {
            ...
            if (eclipse_info_xml_file_state == COULD_NOT_OPEN_FILE) {
                ...
                template_complete_boundary.height *= survivors_ordered.size();
                EndMode2D();
                ...
                // Completion Percent ------------------------------------------ //
                UInt completion_percent = normalize<float>((float)eclipse_level_sum, 0, survivors_ordered.size()*8) * 100.f;
